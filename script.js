...
let dernier = 0;//1 ere element de notre citation
...
function genearteNombreEntier(max){

    return Math.floor(Math.random()* Math.floor(max));

    //0.75 * 10 =7.5   et math.floor retourn 7
}
  button.addEventListener('click',()=>{

   do {
       nombreAleatoire=genearteNombreEntier(citations.length)
   } while (nombreAleatoire == dernier);//Tans que nombre alaetoire est egal a zero
